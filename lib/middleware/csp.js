...
*/
var _policy = module.exports.policy = function (policy) {
    if (policy) {
        csp = {};
...
var violations = {};
module.exports.createServer = function (app, options) {
    var options = options || {};
    var max_length = options.max_length || 100;
    var username = options.username || 'helmet';
    var password = options.password || 'letmein';

    app.get('/helmet/policy', express.basicAuth(username, password), function (req, res) {
        res.render(__dirname + '/../views/policyTool.jade', {pretty: true});
    });

    app.get('/helmet/policy.json', express.basicAuth(username, password), function (req, res) {
        res.setHeader('Content-Type', 'application/javascript');
        res.send(toJSON());
    });

    app.post('/helmet/policy.json', express.basicAuth(username, password), function (req, res) {
        console.log(req.body);
        _policy(req.body);
        res.send("saved");
    });

    app.get('/helmet/js/helmet.js', express.basicAuth(username, password), function (req, res) {
    ...
    app.get('/helmet/css/style.css', express.basicAuth(username, password), function (req, res) {
